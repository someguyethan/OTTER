...
int main() {
	...
	VertexArrayObject::sptr OBJ_vao = VertexArrayObject::Create();
	bool objectLoader = OBJLoader::LoadOBJFile("monkey.obj", position, uv, normal);

	VertexBuffer::sptr vertices_vbo = VertexBuffer::Create();
	vertices_vbo->LoadData(position.data(), position.size());

	VertexBuffer::sptr normals_vbo = VertexBuffer::Create();
	normals_vbo->LoadData(normal.data(), normal.size());

	OBJ_vao->AddVertexBuffer(vertices_vbo, {
		BufferAttribute(0, 3, GL_FLOAT, false, 0, NULL)
		});
	OBJ_vao->AddVertexBuffer(normals_vbo, {
	...
	glm::vec3 lightPos = glm::vec3(0.0f, 3.0f, 0.2f);
	glm::vec3 lightCol = glm::vec3(1.0f, 1.0f, 1.0f);
	float     lightAmbientPow = 0.00f;
	...
	//Translating objects to starting positions
	transform3 = glm::translate(transform3, glm::vec3(1.5f, 1.0f, 0.0));
	transform4 = glm::translate(transform4, glm::vec3(-1.5f, -1.0f, 0.0f));

	//transform4 = glm::rotate(transform4, glm::radians(90.0f), glm::vec3(1.f, 1.f, 1.f));

	//transform4 = glm::scale(transform4, glm::vec3(0.1f, 0.1f, 0.1f));


	camera = Camera::Create();
	camera->SetPosition(glm::vec3(0, 5, 1)); // Set initial position
	camera->SetUp(glm::vec3(0, 0, 1)); // Use a z-up coordinate system
	camera->LookAt(glm::vec3(0.0f)); // Look at center of the screen
	camera->SetFovDegrees(90.0f); // Set an initial FOV

	//Set camera mode variable
	bool isOrtho = false;
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//Rotating objects
		transform3 = glm::rotate(transform3, 0.01f, glm::vec3(0.0f, 0.0f, 1.0f));
		transform4 = glm::rotate(transform4, 0.01f, glm::vec3(0.0f, 0.0f, 1.0f));
		...
		else if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_RELEASE && spacePressed == true)
		{
			spacePressed = false;
		}
