...
uniform sampler2D s_Reflectiveness;
...
void main() {
	...
	//Do reflectiveness stuff
	vec3 toEye = normalize(inPos - u_CamPos);
	vec3 reflected = reflect(toEye, N);
	float texRef = texture(s_Reflectiveness, inUV).y;
	...
	vec3 result = mix((
		...
		) * inColor * textureColor.rgb, environment, texRef); // Object color
