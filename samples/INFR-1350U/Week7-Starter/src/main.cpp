...
int main() {
	...
	imGuiCallbacks.push_back([&]() {
		...
		if (ImGui::CollapsingHeader("Material Level Lighting Settings"))
		{
			...
			if (ImGui::SliderFloat("TextureMix", &textureMix, 0.0f, 1.0f)) {
				shader->SetUniform("u_TextureMix", textureMix);
			}
		}
	});
	...
	Texture2DData::sptr texture2Map = Texture2DData::LoadFromFile("images/box.bmp");
	...
	Texture2D::sptr texture2 = Texture2D::Create();
	texture2->LoadData(texture2Map);
	...
	keyToggles.emplace_back(GLFW_KEY_K, [&]() {
		selectedVao--;
		if (selectedVao <= 0)
			selectedVao = 3;
	});
	...
	while (!glfwWindowShouldClose(window)) {
		...
		shader->SetUniform("s_Texture2", 2);
